...
export default {
	async fetch(request, env, ctx) {
		...
		try {
            ...
            if (url.pathname === '/simulator/v2/getComponentDetail' && request.method === 'GET') {
			...
			if (url.pathname === '/simulator/v2/getComponentList' && request.method === 'GET') {
                const type = parseInt(url.searchParams.get('type'));
                const page = parseInt(url.searchParams.get('page')) || 1;
                const pageSize = parseInt(url.searchParams.get('page_size')) || 10;

				if (!type || !TYPE_TO_MANIFEST[type]) {
					console.error(`[API_WARN] Tipe komponen tidak valid: ${type}`);
			...
			if (url.pathname.startsWith('/simulator/v2/')) {
				...
				if (response.ok) {
					...
					console.error(`[V2_MISSING] File tidak ditemukan: ${realPath}`);
			...
			if (githubResponse.ok) {
				...
				console.error(`[OTHER_MISSING] File tidak ditemukan: ${url.pathname}`);
				return createStubResponse(null, 404, 'Not Found');
			}
